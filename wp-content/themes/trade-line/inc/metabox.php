...
if ( ! function_exists( 'trade_line_save_theme_settings_meta' ) ) :
	...
	function trade_line_save_theme_settings_meta( $post_id, $post ) {
		...
		// Verify nonce.
		if (
			! ( isset( $_POST['trade_line_theme_settings_meta_box_nonce'] )
			&& wp_verify_nonce( sanitize_key( $_POST['trade_line_theme_settings_meta_box_nonce'] ), basename( __FILE__ ) ) )
		) {
			return;
		}
...
add_action( 'save_post', 'trade_line_save_theme_settings_meta', 10, 2 );
