...
function trade_line_scripts() {
	...
	wp_enqueue_style( 'trade-line-style', get_stylesheet_uri(), array(), '1.6.2' );

	if ( has_header_image() ) {
		$extra_css = '#masthead{ background-image: url("' . esc_url( get_header_image() ) . '"); background-repeat: no-repeat; background-position: center center; }';
		wp_add_inline_style( 'trade-line-style', $extra_css );
	}
